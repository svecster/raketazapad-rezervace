-- Add remaining test data to reach exactly 10 records for each table
...
-- Clean checkouts to keep exactly 10
WITH checkouts_to_keep AS (
  SELECT id FROM public.checkouts ORDER BY created_at LIMIT 10
)
DELETE FROM public.checkouts WHERE id NOT IN (SELECT id FROM checkouts_to_keep);

-- Add 3 more checkouts to reach 10 total (if needed)
DO $$
DECLARE
...
BEGIN
    ...
    INSERT INTO public.checkouts (reservation_id, staff_user_id, total_amount, status, include_court_price, notes, created_at, updated_at) 
    SELECT reservation_id, staff_user_id, total_amount, status, include_court_price, notes, created_at, updated_at FROM (VALUES
      (reservation_ids[1], staff_ids[1], 525, 'completed', true, 'Rezervace + občerstvení', '2024-12-23 10:30:00+01'::timestamp, '2024-12-23 10:45:00+01'::timestamp),
      (reservation_ids[2], staff_ids[2], 400, 'completed', true, 'Jen rezervace kurtu', '2024-12-23 12:15:00+01'::timestamp, '2024-12-23 12:20:00+01'::timestamp),
      (reservation_ids[3], staff_ids[1], 715, 'completed', true, 'Premium rezervace + nápoje', '2024-12-23 14:15:00+01'::timestamp, '2024-12-23 14:30:00+01'::timestamp)
    ) AS new_checkouts(reservation_id, staff_user_id, total_amount, status, include_court_price, notes, created_at, updated_at)
    WHERE (SELECT COUNT(*) FROM public.checkouts) < 10
    LIMIT (10 - (SELECT COUNT(*) FROM public.checkouts));
END $$;