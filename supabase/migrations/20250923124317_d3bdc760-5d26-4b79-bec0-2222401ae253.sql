-- Fix existing court data and create comprehensive data model for tennis facility MVP

-- First update existing courts data to match new constraints
...
DROP CONSTRAINT IF EXISTS check_app_role;
...
DROP CONSTRAINT IF EXISTS check_reservation_status;
...
-- Create new products table (rename old one first if exists)
...
-- Create new payments table (rename old one first if exists)
...
-- Insert some sample price rules
...
RETURNS TRIGGER AS $$
BEGIN
    -- Only create bar tab if reservation has a user (not for guest reservations without user)
