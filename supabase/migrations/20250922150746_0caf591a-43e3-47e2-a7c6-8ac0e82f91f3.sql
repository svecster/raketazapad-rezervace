-- Safety SQL fixes and seed data for POS system
...
-- 2) Safe helper function for RLS (reads role from JWT > user_metadata > user_profiles)
...
DECLARE
  ...
  resolved text;
BEGIN
  -- try JWT claim app_role (auth.jwt() ->> 'app_role')
  BEGIN
  ...
  -- try user_metadata (auth.users)
  SELECT (raw_user_meta_data->>'app_role')
  INTO meta
  FROM auth.users
  WHERE id = uid;

  -- try user_profiles.app_role
  SELECT app_role
  INTO prof
...
-- 3) Fix reservation_id type in sales table
DO $$
BEGIN
