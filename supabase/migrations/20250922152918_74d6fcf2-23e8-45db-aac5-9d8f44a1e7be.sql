...
-- Create function to get app role from user_profiles (returns text, not enum)
...
USING (
  auth.uid() = user_id OR 
  public.get_current_user_app_role() IN ('admin', 'owner')
);
...
FOR UPDATE 
USING (public.get_current_user_app_role() IN ('admin', 'owner')) 
WITH CHECK (true);
